...
fn test_connect_block_rejects_bip30_violation() {
    ...
    let result = { let ctx = block::BlockValidationContext::for_network(types::Network::Mainnet); connect_block(&block, &witnesses, utxo_set, 1, &ctx) };
...
fn test_connect_block_rejects_bip34_violation() {
    ...
    let result = { let ctx = block::BlockValidationContext::for_network(types::Network::Mainnet); connect_block(&block, &witnesses, utxo_set, height, &ctx) };
