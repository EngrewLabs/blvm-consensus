...
fn test_rbf_fee_bump_requirements() {
    ...
    assert_eq!(
        existing_tx.inputs[0].prevout.hash,
        new_tx.inputs[0].prevout.hash
    );
    assert_eq!(
        existing_tx.inputs[0].prevout.index,
        new_tx.inputs[0].prevout.index
    );
}
...
fn test_rbf_all_five_rules() {
    ...
    assert_eq!(
        existing_tx.inputs[0].prevout,
        valid_replacement.inputs[0].prevout
    );
}
