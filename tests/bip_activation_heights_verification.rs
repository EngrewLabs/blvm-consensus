...
//! - BIP34: Coinbase height (activation at 227,931 mainnet; Bitcoin Core `BIP34Height`)
...
///
/// BIP34 activates at height 227,931 (mainnet)
#[test]
fn test_bip34_before_activation() {
    ...
    assert!(
        height < BIP34_ACTIVATION_MAINNET,
...
///
/// BIP34 enforced after height 227,931 (mainnet)
#[test]
...
fn test_bip147_before_activation() {
    let height = BIP147_ACTIVATION_MAINNET - 1;
    let _network = Network::Mainnet;
...
fn test_bip147_at_activation() {
    let height = BIP147_ACTIVATION_MAINNET;
    let _network = Network::Mainnet;
...
fn test_bip_activation_heights_match_consensus() {
    // Verify heights are in ascending order (single source of truth: blvm-primitives constants)
    assert!(
        BIP34_ACTIVATION_MAINNET < BIP66_ACTIVATION_MAINNET,
        "BIP34 should activate before BIP66"
    );
    assert!(
        BIP66_ACTIVATION_MAINNET < BIP147_ACTIVATION_MAINNET,
