...
fn benchmark_transaction_validation(c: &mut Criterion) {
    ...
    c.bench_function("check_transaction", |b| {
        b.iter(|| {
            black_box(check_transaction(black_box(&tx)))
        })
    });
}
...
fn benchmark_transaction_validation_complex(c: &mut Criterion) {
    ...
    let tx = Transaction {
        ...
        outputs: (0..10).map(|i| TransactionOutput {
...
criterion_group!(benches, benchmark_transaction_validation, benchmark_transaction_validation_complex);
