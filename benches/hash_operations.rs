...
fn benchmark_batch_sha256(c: &mut Criterion) {
    ...
    for size in batch_sizes {
        ...
        c.bench_function(&format!("batch_sha256_{}", size), |b| {
            b.iter(|| {
                black_box(simd_vectorization::batch_sha256(black_box(&input_refs)))
            })
        });
        ...
        c.bench_function(&format!("sequential_sha256_{}", size), |b| {
            b.iter(|| {
                let results: Vec<[u8; 32]> = inputs.iter().map(|data| {
...
fn benchmark_batch_double_sha256(c: &mut Criterion) {
    ...
    for size in batch_sizes {
        ...
        c.bench_function(&format!("batch_double_sha256_{}", size), |b| {
            b.iter(|| {
                black_box(simd_vectorization::batch_double_sha256(black_box(&input_refs)))
            })
        });
...
fn benchmark_batch_hash160(c: &mut Criterion) {
    ...
    for size in batch_sizes {
        ...
        c.bench_function(&format!("batch_hash160_{}", size), |b| {
            b.iter(|| {
                black_box(simd_vectorization::batch_hash160(black_box(&input_refs)))
            })
        });
        ...
        c.bench_function(&format!("sequential_hash160_{}", size), |b| {
            b.iter(|| {
                let results: Vec<[u8; 20]> = inputs.iter().map(|data| {
...
fn benchmark_merkle_root_batching(c: &mut Criterion) {
    use consensus_proof::{Transaction, TransactionInput, TransactionOutput, OutPoint};
    ...
    for tx_count in tx_counts {
        ...
        let transactions: Vec<Transaction> = (0..tx_count).map(|i| Transaction {
            version: 1u64,
            inputs: vec![TransactionInput {
                prevout: OutPoint { hash: [i as u8; 32], index: 0u64 },
        ...
        c.bench_function(&format!("merkle_root_{}tx", tx_count), |b| {
            b.iter(|| {
                black_box(calculate_merkle_root(black_box(&transactions)).unwrap())
            })
        });
    }
}
...
fn benchmark_block_validation_tx_ids(c: &mut Criterion) {
    ...
    for tx_count in tx_counts {
        ...
        c.bench_function(&format!("batch_tx_id_{}tx", tx_count), |b| {
            b.iter(|| {
                let serialized: Vec<Vec<u8>> = transactions.iter().map(|tx| serialize_transaction(tx)).collect();
...
fn benchmark_sighash_batching(c: &mut Criterion) {
    use consensus_proof::{Transaction, TransactionInput, TransactionOutput, OutPoint};
    use consensus_proof::transaction_hash::{batch_compute_sighashes, SighashType};
    ...
    for input_count in input_counts {
        ...
        let prevouts: Vec<TransactionOutput> = (0..input_count).map(|_| TransactionOutput {
        ...
        c.bench_function(&format!("batch_sighash_{}inputs", input_count), |b| {
            b.iter(|| {
                black_box(batch_compute_sighashes(black_box(&tx), black_box(&prevouts), SighashType::All).unwrap())
            })
        });
    }
}
...
fn benchmark_pow_batching(c: &mut Criterion) {
    ...
    use consensus_proof::pow::batch_check_proof_of_work;
    
    let header_counts = vec![8, 16, 32, 64, 128];
    
    for count in header_counts {
        let headers: Vec<BlockHeader> = (0..count).map(|i| BlockHeader {
            version: 1i64,
        ...
        c.bench_function(&format!("batch_pow_{}headers", count), |b| {
            b.iter(|| {
                black_box(batch_check_proof_of_work(black_box(&headers)).unwrap())
            })
        });
    }
}
...
fn benchmark_batch_ecdsa_verification(c: &mut Criterion) {
    use consensus_proof::script::batch_verify_signatures;
    ...
    for count in signature_counts {
        ...
        c.bench_function(&format!("batch_verify_signatures_{}", count), |b| {
            b.iter(|| {
                black_box(batch_verify_signatures(black_box(&verification_tasks)))
            })
        });
        ...
        c.bench_function(&format!("sequential_verify_signatures_{}", count), |b| {
            b.iter(|| {
                ...
                let results: Vec<bool> = verification_tasks.iter().map(|(pubkey, sig, hash)| {
                    match (
                        secp256k1::PublicKey::from_slice(pubkey),
                        Signature::from_der(sig),
                        Message::from_digest_slice(hash),
                    ) {
                        (Ok(pk), Ok(sig), Ok(msg)) => secp.verify_ecdsa(&msg, &sig, &pk).is_ok(),
                        _ => false,
                    }
                }).collect();
                black_box(results)
...
fn benchmark_sighash_templates(c: &mut Criterion) {
    use consensus_proof::{Transaction, TransactionInput, TransactionOutput, OutPoint};
    use consensus_proof::transaction_hash::{calculate_transaction_sighash, SighashType};
    
    // Create standard transaction (1 input, 1 output) - most common pattern
    let tx = Transaction {
        version: 1u64,
        inputs: vec![TransactionInput {
            prevout: OutPoint { hash: [0u8; 32], index: 0u64 },
            script_sig: vec![0x51],
            sequence: 0xffffffffu64,
        }],
        outputs: vec![TransactionOutput {
    ...
    c.bench_function("sighash_with_template_check", |b| {
        b.iter(|| {
            black_box(calculate_transaction_sighash(black_box(&tx), 0, black_box(&prevouts), SighashType::All).unwrap())
        })
    });
}
...
fn benchmark_early_exit_transaction(c: &mut Criterion) {
    ...
    c.bench_function("check_transaction_invalid_fast_path", |b| {
        b.iter(|| {
            black_box(check_transaction(black_box(&invalid_tx)).unwrap())
        })
    });
    
    c.bench_function("check_transaction_valid_full", |b| {
        b.iter(|| {
            black_box(check_transaction(black_box(&valid_tx)).unwrap())
        })
    });
}
...
criterion_group!(benches, benchmark_sha256, benchmark_double_sha256);
