...
use bllvm_consensus::{
    Block, BlockHeader, Transaction, TransactionInput, TransactionOutput, OutPoint, UtxoSet, UTXO,
    block::connect_block,
    segwit::Witness,
};
...
fn bench_hash_batch(c: &mut Criterion) {
    ...
    for size in [8, 16, 32, 64, 128].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("avx2_double_sha256", size), size, |b, _| {
            b.iter(|| {
                black_box(simd_vectorization::batch_double_sha256(black_box(&inputs)))
            })
        });
    }
...
fn bench_transaction_basics(c: &mut Criterion) {
    ...
    group.bench_function("calculate_id", |b| {
        ...
        b.iter(|| {
            black_box(calculate_tx_id(black_box(&tx)))
        })
    });
...
fn bench_utxo_operations(c: &mut Criterion) {
    ...
    group.bench_function("get", |b| {
        b.iter(|| {
            black_box(utxo_set.get(black_box(&outpoint)))
        })
    });
