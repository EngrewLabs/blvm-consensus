...
use crate::types::{OutPoint, UTXO, Hash, Natural};
#[cfg(feature = "utxo-commitments")]
...
impl UtxoMerkleTree {
    ...
    pub fn new() -> UtxoCommitmentResult<Self> {
        ...
        let tree = SparseMerkleTree::new_with_store(store)
            .map_err(|e| UtxoCommitmentError::MerkleTreeError(format!("Failed to create tree: {:?}", e)))?;
    ...
    pub fn insert(&mut self, outpoint: OutPoint, utxo: UTXO) -> UtxoCommitmentResult<Hash> {
        ...
        // Update tree
        let root_h256 = self.tree.update(key, utxo_value)
    ...
    pub fn remove(&mut self, outpoint: &OutPoint, utxo: &UTXO) -> UtxoCommitmentResult<Hash> {
        ...
        let root_h256 = self.tree.update(key, zero_value)
    ...
    pub fn generate_proof(&self, outpoint: &OutPoint) -> UtxoCommitmentResult<sparse_merkle_tree::MerkleProof> {
        let key = self.hash_outpoint(outpoint);
        let keys = vec![key];
        
        self.tree.merkle_proof(keys)
            .map_err(|e| UtxoCommitmentError::MerkleTreeError(format!("Failed to generate proof: {:?}", e)))
    }
    ...
    fn deserialize_utxo(&self, data: &[u8]) -> UtxoCommitmentResult<UTXO> {
        if data.len() < 17 {
            return Err(UtxoCommitmentError::InvalidUtxo("Data too short".to_string()));
        }
        ...
        if data.len() < offset + script_len {
            return Err(UtxoCommitmentError::InvalidUtxo("Script length mismatch".to_string()));
        }
