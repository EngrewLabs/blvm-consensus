...
impl std::fmt::Display for TransactionParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            TransactionParseError::InsufficientBytes => write!(f, "Insufficient bytes to parse transaction"),
...
pub fn deserialize_transaction(data: &[u8]) -> Result<Transaction> {
    ...
    let version = i32::from_le_bytes([data[offset], data[offset + 1], data[offset + 2], data[offset + 3]]) as u64;
    offset += 4;
...
mod tests {
    ...
    fn test_serialize_deserialize_round_trip() {
        ...
        assert_eq!(deserialized.inputs[0].prevout.hash, tx.inputs[0].prevout.hash);
        assert_eq!(deserialized.inputs[0].prevout.index, tx.inputs[0].prevout.index);
        ...
        assert_eq!(deserialized.outputs[0].script_pubkey, tx.outputs[0].script_pubkey);
...
mod kani_proofs {
    ...
    fn kani_transaction_serialization_round_trip() {
        ...
        if deserialized_result.is_ok() {
            ...
            // Verify inputs match
            for (i, (input, deserialized_input)) in tx.inputs.iter().zip(deserialized.inputs.iter()).enumerate() {
                assert_eq!(deserialized_input.prevout.hash, input.prevout.hash,
                    "Transaction serialization round-trip: input {} prevout hash must match", i);
                assert_eq!(deserialized_input.prevout.index, input.prevout.index,
                    "Transaction serialization round-trip: input {} prevout index must match", i);
                assert_eq!(deserialized_input.script_sig, input.script_sig,
                    "Transaction serialization round-trip: input {} script_sig must match", i);
                assert_eq!(deserialized_input.sequence, input.sequence,
                    "Transaction serialization round-trip: input {} sequence must match", i);
            }
            
            // Verify outputs match
            for (i, (output, deserialized_output)) in tx.outputs.iter().zip(deserialized.outputs.iter()).enumerate() {
                assert_eq!(deserialized_output.value, output.value,
                    "Transaction serialization round-trip: output {} value must match", i);
                assert_eq!(deserialized_output.script_pubkey, output.script_pubkey,
                    "Transaction serialization round-trip: output {} script_pubkey must match", i);
            }
        }
    }
