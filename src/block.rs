...
use crate::segwit::{Witness, validate_witness_commitment, compute_witness_merkle_root, is_segwit_transaction};
...
pub fn connect_block(
    ...
    if witnesses.len() != block.transactions.len() {
        return Ok((ValidationResult::Invalid(
            format!("Witness count {} does not match transaction count {}", witnesses.len(), block.transactions.len())
        ), utxo_set));
    }
    ...
    for (i, tx) in block.transactions.iter().enumerate() {
        ...
        // Verify scripts for non-coinbase transactions
            ...
            if !is_coinbase(tx) && !skip_signatures {
                ...
                let prevouts: Vec<TransactionOutput> = tx.inputs
                    .iter()
                    .filter_map(|input| {
                    ...
                    .map(|(j, opt_script_pubkey)| {
                        if let Some(script_pubkey) = opt_script_pubkey {
                            ...
                            let median_time_past = recent_headers
                                .map(|headers| get_median_time_past(headers))
    ...
    if let Some(coinbase) = block.transactions.first() {
        ...
        if coinbase.inputs[0].script_sig.len() < 2 || coinbase.inputs[0].script_sig.len() > 100 {
            return Ok((ValidationResult::Invalid(
                format!("Coinbase scriptSig length {} must be between 2 and 100 bytes", 
        ...
        let coinbase_output: i64 = coinbase.outputs.iter()
            .try_fold(0i64, |acc, output| {
                acc.checked_add(output.value)
                    .ok_or_else(|| ConsensusError::BlockValidation(
                        "Coinbase output value overflow".to_string()
                    ))
            })
        ...
        if coinbase_output > MAX_MONEY {
            return Ok((ValidationResult::Invalid(
                format!("Coinbase output {} exceeds maximum money supply", coinbase_output)
            ), utxo_set));
        }
        
        // Use checked arithmetic for fee + subsidy calculation
        let max_coinbase_value = total_fees.checked_add(subsidy)
            .ok_or_else(|| ConsensusError::BlockValidation(
                "Fees + subsidy overflow".to_string()
            ))?;
        
        if coinbase_output > max_coinbase_value {
            return Ok((ValidationResult::Invalid(
                format!("Coinbase output {} exceeds fees {} + subsidy {}", coinbase_output, total_fees, subsidy)
            ), utxo_set));
        }
    ...
    let flags = calculate_script_flags_for_block(block.transactions.first().unwrap(), 
                                                   witnesses.first());
    
    for (i, tx) in block.transactions.iter().enumerate() {
        let tx_witness = witnesses.get(i);
        
        let tx_sigop_cost = get_transaction_sigop_cost(
            tx,
            &utxo_set,
            tx_witness,
            flags
        ...
        total_sigop_cost = total_sigop_cost.checked_add(tx_sigop_cost)
            .ok_or_else(|| ConsensusError::BlockValidation(
                format!("Sigop cost overflow at transaction {}", i)
            ))?;
    }
    
    if total_sigop_cost > MAX_BLOCK_SIGOPS_COST {
        return Ok((ValidationResult::Invalid(
            format!("Block sigop cost {} exceeds maximum {}", total_sigop_cost, MAX_BLOCK_SIGOPS_COST)
        ), utxo_set));
    }
...
pub(crate) fn calculate_script_flags_for_block(tx: &Transaction, tx_witness: Option<&Witness>) -> u32 {
    // Base flags (standard validation flags)
...
mod kani_proofs {
    ...
    fn kani_apply_transaction_with_id_correctness() {
        ...
        if !is_coinbase(&tx) {
            for input in &tx.inputs {
                if !utxo_set_with_inputs.contains_key(&input.prevout) {
                    utxo_set_with_inputs.insert(input.prevout.clone(), UTXO {
                        value: 1000,
                        script_pubkey: vec![],
        ...
        if result1.is_ok() && result2.is_ok() {
            ...
            assert_eq!(utxo_set1.len(), utxo_set2.len(),
    ...
    fn kani_apply_transaction_mathematical_correctness() {
        ...
        let initial_utxos: Vec<(OutPoint, UTXO)> = utxo_set.iter()
            ...
            Ok(new_utxo_set) => {
                if is_coinbase(&tx) {
                    ...
                    for (outpoint, utxo) in &initial_utxos {
                        ...
                        assert_eq!(new_utxo_set.get(outpoint), Some(utxo),
                    ...
                    assert_eq!(new_utxo_set.len(), initial_size + tx.outputs.len(),
                    ...
                    let expected_size = (initial_size as i64) - 
                        (tx.inputs.len() as i64) + 
                        (tx.outputs.len() as i64);
                    assert_eq!(new_utxo_set.len() as i64, expected_size,
                ...
                if !is_coinbase(&tx) {
                    ...
                    for input in &tx.inputs {
                        ...
                        if second_apply.is_ok() {
                            ...
                            assert!(!second_set.contains_key(&input.prevout),
    ...
    fn kani_script_flags_calculation_correctness() {
        ...
        let base_flags = 0x01 | 0x02 | 0x04 | 0x08 | 0x10 | 0x20 | 0x40 | 0x80 | 0x100 | 0x200 | 0x400;
        assert!(flags & base_flags == base_flags,
    ...
    fn kani_validate_block_header_complete() {
        ...
        assert!(!result.unwrap_or(true), "Header with version 0 must be invalid");
        
        header.version = 1;
        header.bits = 0;
        let result = validate_block_header(&header);
        assert!(!result.unwrap_or(true), "Header with bits = 0 must be invalid");
        
        header.bits = 0x1d00ffff;
        header.merkle_root = [0u8; 32];
        let result = validate_block_header(&header);
        assert!(!result.unwrap_or(true), "Header with zero merkle_root must be invalid");
        ...
        if result.is_ok() {
            assert!(result.unwrap() || true, "Valid header structure should pass");
        }
    }
    ...
    fn kani_transaction_id_uniqueness() {
        ...
        assert_eq!(id1, id1_repeat,
            "Same transaction must produce same ID");
    }
            ...
            Ok((ValidationResult::Valid, _)) => {
                ...
                if let Some(coinbase) = block.transactions.first() {
                    if coinbase.outputs.len() > 0 {
                        ...
                        assert!(coinbase_output <= max_allowed || max_allowed < 0,
...
mod kani_proofs {
        ...
        if result.is_ok() {
            ...
            if matches!(validation_result, ValidationResult::Valid) {
                ...
                if !block.transactions.is_empty() {
                    ...
                    let has_outputs = block.transactions.iter()
                        .any(|tx| !tx.outputs.is_empty());
                    if has_outputs {
    ...
    fn kani_block_size_limits() {
        ...
        assert!(approximate_block_size <= MAX_BLOCK_SIZE || block.transactions.is_empty(),
        ...
        assert!(serialized_header.len() == 80,
    ...
    fn kani_block_weight_limits() {
        ...
        if weight_result.is_ok() {
            ...
            assert!(weight <= max_weight as Natural || block.transactions.is_empty(),
                "Block weight must not exceed W_max = 4,000,000 (DoS prevention)");
            
            // Weight must be positive
            assert!(weight > 0 || block.transactions.is_empty(),
    ...
    fn kani_fee_accumulation_overflow_safety() {
        ...
        assert!(total_fees <= MAX_MONEY * (block.transactions.len() as i64),
            ...
            if matches!(validation_result, ValidationResult::Valid) {
                ...
                if !block.transactions.is_empty() {
                    ...
                    if !coinbase.outputs.is_empty() {
                        let total_output: i64 = coinbase.outputs.iter()
                            .map(|output| output.value)
                            .sum();
                        
                        // Output must be non-negative and not exceed reasonable bounds
                        assert!(total_output >= 0,
                            "Coinbase output must be non-negative");
                        assert!(total_output <= MAX_MONEY,
                    ...
                    assert!(coinbase.inputs[0].script_sig.len() >= 2,
